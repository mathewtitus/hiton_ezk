...
def eliminate(x, y, TPC, system, params):
    ...
    if verbose: print("calling eliminate({},{})\nTPC: {}\ntarget var: {}".format(x,y,TPC,params['sampling_params']['target_variable']))
    ...
    for sub in tpc_subsets:
        ...
        for pair in sample_pairs:
            ...
            if verbose:
                print("conditions:\nk: {}\nl: {}".format(sub_conds_kay, sub_conds_ell))
            ...
            n = ax.hist([x_draws[:params['N1']], x_draws[params['N1']:]], bins=np.linspace(-np.pi,np.pi - np.pi/8,15))
            ax.title.set_text("c: {}\ndist: {}\nDiff: {}".format(
                np.round(c_xyK[-1], 4),
                np.round(d_y1_y2, 4),
